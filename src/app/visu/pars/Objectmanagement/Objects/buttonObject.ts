...
export function createButtonObject(
    ...
    const absCornerCoord = {
        x1: buttonShape.rect[0],
        y1: buttonShape.rect[1],
        x2: buttonShape.rect[2],
        y2: buttonShape.rect[3],
    };
    // absCenterCoord are the coordinates of the rotation and scale center
    const absCenterCoord = {
        x: buttonShape.center[0],
        y: buttonShape.center[1],
    };
    // relCoord are the width and the height in relation the div
    const relCoord = {
        width: buttonShape.rect[2] - buttonShape.rect[0],
        height: buttonShape.rect[3] - buttonShape.rect[1],
    };
    // the relCenterCoord are the coordinates of the midpoint of the div
    const relMidpointCoord = {
        x: (buttonShape.rect[2] - buttonShape.rect[0]) / 2,
        y: (buttonShape.rect[3] - buttonShape.rect[1]) / 2,
    };
    ...
    const edge =
        buttonShape.lineWidth === 0 ? 1 : buttonShape.lineWidth;
    // Compute the strokeWidth through hasFrameColor
    const lineWidth = buttonShape.hasFrameColor ? edge : 0;
    // Compute the fill color through has_fill_color
    const fillColor = buttonShape.hasInsideColor
        ? buttonShape.fillColor
        : 'none';
    // Tooltip
    const tooltip = buttonShape.tooltip;

    const shape = buttonShape.shape;
    ...
    const initial: IBasicObject = {
        ...
        normalFillColor: buttonShape.fillColor,
        alarmFillColor: buttonShape.fillColorAlarm,
        normalFrameColor: buttonShape.frameColor,
        alarmFrameColor: buttonShape.frameColorAlarm,
        hasFillColor: buttonShape.hasInsideColor,
        hasFrameColor: buttonShape.hasFrameColor,
        lineWidth: lineWidth,
    ...
    // Polyshapes and buttonShapes have the same <expr-...> possibilities
    ...
    // We have to note that the rotation of polylines is not the same like buttonShapes. buttonShapes keep their originally alignment, polyhapes transform every coordinate.
    ...
    // The transformed corner coordinates depends on the shapetype. The rotating operation is different for buttonShapes and polyshapes
    // buttonShape:
    ...
    Object.defineProperty(initial, 'writeAccess', {
        get: function () {
            ...
            if (!isNaN(currentNum)) {
                if (
                    buttonShape.accessLevels[currentNum].includes('w')
                ) {
                    return true;
                } else {
                    return false;
    ...
    Object.defineProperty(initial, 'readAccess', {
        get: function () {
            ...
            if (!isNaN(currentNum)) {
                if (
                    buttonShape.accessLevels[currentNum].includes('r')
                ) {
                    return true;
                } else {
                    return false;
