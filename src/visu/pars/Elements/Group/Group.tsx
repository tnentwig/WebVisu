...
{   
    ...
    for (let i=0; i<section.children.length; i++){
        ...
        if (element.nodeName === "element"){
            ...
            switch(type) {
                case "simple":
                    addVisuObject(<SimpleShape section={element}></SimpleShape>);
                    getDimension(rightDownCorner, stringToArray(element.getElementsByTagName("rect")[0].innerHTML));
                    break;
                case "polygon":
                    addVisuObject(<PolyShape section={element}></PolyShape>);
                    let points = element.getElementsByTagName('point');
                    for (let i=0; i<points.length; i++){
                        getDimension(rightDownCorner, stringToArray(points[i].innerHTML));
                    };
                    break;
                case "button":
                    addVisuObject(<Button section={element}></Button>);
                    break;
                case "group":
