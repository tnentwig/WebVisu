...
export function createPolyObject(polyShape: IPolyShape, dynamicElements : Map<string,string[][]>) : IPolyObject{
    ...
    let absCornerCoord = {x1:polyShape.rect[0],y1:polyShape.rect[1],x2:polyShape.rect[2],y2:polyShape.rect[3]};
    // absCenterCoord are the coordinates of the rotation and scale center
    let absCenterCoord = {x:polyShape.center[0], y:polyShape.center[1]};
    // relCoord are the width and the height in relation the div
    let relCoord = {width:polyShape.rect[2]-polyShape.rect[0], height:polyShape.rect[3]-polyShape.rect[1]};
    // the relCenterCoord are the coordinates of the midpoint of the div
    let relMidpointCoord = {x:(polyShape.rect[2]-polyShape.rect[0])/2, y:(polyShape.rect[3]-polyShape.rect[1])/2};
    ...
    let fillColor = (polyShape.has_inside_color) ? polyShape.fill_color : 'none';
    // Tooltip
